...
**当前状态**:SentinelCore + AppLogic 逻辑层完成并加固,UI 层完成并可编译。

---

## 打包收尾说明(2026-07-03,Task 17)

- 产出 `Resources/Info.plist`(`LSUIElement`、bundle id `com.wenzhurong.v2rayn-sentinel`、最低 14.0)、`scripts/make-app.sh`、`scripts/feed-log.sh`。
- **make-app.sh 的 locale 修复**:本机为非 UTF-8 locale,原计划脚本里 `$DEST…` 的省略号 `…`(U+2026)紧贴变量名,被 bash 并入变量名导致 `unbound variable`。已改为纯 ASCII 标点 + `${VAR}` 花括号定界。
- `./scripts/make-app.sh` 已成功组装 `build/V2rayN Sentinel.app`(`Mach-O arm64` 可执行 + Info.plist 校验通过);`build/` 在 `.gitignore`,不入库。
- **端到端手动验证(计划 Task 17 Step 5)按用户要求暂缓,未运行 App**。待运行时:用 `logDirOverride` 指向临时目录 → `feed-log.sh <dir> ordinary` 看左上角小 toast(5s 消失、无声)→ `feed-log.sh <dir> important` 看红色卡片 + Basso + 需手动关 → 菜单栏图标/历史/设置逐项核对。
